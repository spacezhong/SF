...
function KindContent(props){

    const renderLeft=()=>{
        let dataSource=props.dataSource || [];
        return (
            dataSource.map((item,leftIndex)=>{
                let cls=(leftIndex===props.currentLeftIndex)?'kindContent-left-item active':'kindContent-left-item';
                return(
    ...
    const handleLeftClick=(leftIndex)=>{
        props.clickLeftAPI(leftIndex);
    };
    const renderRight=()=>{
        let currentLeftIndex=props.currentLeftIndex;//拿到左边当前高亮项的索引
        let dataSource=props.dataSource || [];//拿到总数组
            ...
            return(
                [
                    ...
                    <div className="right-list-inner" key={2}>
                        {renderRightList(currentData.right)}
                    </div>
                ]
            )
        }else{
    ...
    const renderRightList=(rightData)=>{//rightData=[{"title":'肉类海鲜'},{"title":'新鲜果蔬'},...]
...
export default memo(KindContent);
